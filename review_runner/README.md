...
1. `.github/workflows/terraform-ci.yml` runs every deterministic Pull Request check: branch naming, Terraform formatting and validation, Checkov, repository acceptance tests, and the offline reviewer tests.
2. `.github/workflows/ai-review-after-ci.yml` runs from the default branch after `Terraform CI` completes. Its job exists only when the completed run is a successful Pull Request run with resolved PR metadata.
...
3. Failing branch validation, Terraform formatting or validation, Checkov, acceptance tests, or reviewer tests leaves the AI workflow unstarted and does not consume provider quota.
